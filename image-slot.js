...
(() => {
  ...
  // on SVG blobs is inconsistent). GIFs are stored as-is (no canvas
  // re-encode) so animation is preserved.
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      try {
        ...
        // GIFs bypass the canvas re-encode: drawing to a canvas would keep
        // only the first frame, silently freezing the animation. Store the
        // original bytes as a data URL instead.
